...
neg_rows = df[df['tag'] == 'negative']
pos_rows = df[df['tag'] == 'positive']

print(f'before train test split. neg rows: {len(neg_rows)}, pos rows: {len(pos_rows)}')
...
# neg_count = (df['tag'] == 'negative').sum()
# pos_count = (df['tag'] == 'positive').sum()
